...
export default function System(){
 ...
 async function load(){
  ...
  setHealth(h);setCounts({docs:docs.count||0,articles:arts.count||0,watchlists:watch.count||0,alerts:alerts.count||0});setRefreshing(false);
 }
 useEffect(()=>{load()},[]);
 const c=health?.components||{};
 const items=[
  ["공식자료 URL 수집",c.officialCollector==="manual_url"?"연결됨":"확인 필요"],
  ["자동 순회 수집","다음 단계"],
  ["중복·변경 감지","연결됨"],
 ...
 <section className="adminPanel"><div className="adminSectionTitle"><div><small>HEALTH</small><h2>서비스 연결상태</h2></div><button className="miniBtn" onClick={load} disabled={refreshing}>{refreshing?"확인 중…":"새로고침"}</button></div><div className="systemList">{items.map(([a,b])=><div key={a}><b>{a}</b><span>{b}</span></div>)}</div><p className="adminDataNote">연결되지 않은 외부 서비스는 정상으로 표시하지 않습니다.</p></section>
 ...
 <section className="adminPanel"><div className="adminSectionTitle"><div><small>TOOLS</small><h2>운영 도구</h2></div></div><div className="adminMenu"><a href="/focus-admin/ai-test"><div><b>공식자료 수집·AI 기사작성</b><span>정부·공공기관 공식 URL을 직접 불러와 기사 후보를 만듭니다.</span></div><strong>›</strong></a><a href="/focus-admin/publish"><div><b>검증·발행 대기열</b><span>출처·사실·숫자·날짜·중복·법적검토를 통과한 기사만 발행합니다.</span></div><strong>›</strong></a></div></section>

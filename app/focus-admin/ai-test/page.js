...
export default function AITest(){
 ...
 useEffect(()=>{const doc=new URLSearchParams(location.search).get("doc");if(!doc)return;(async()=>{setImporting(true);try{const{data:d,error}=await dfSupabase.from("df_source_documents").select("title,source_url,content_text").eq("id",doc).single();if(error)throw error;const fallback=`[공식자료] ${d.title}\n제목: ${d.title}${d.source_url?`\n공식URL: ${d.source_url}`:""}\n\n${d.content_text||""}`;setS(fallback);if(d.source_url){setSourceUrl(d.source_url);try{const token=await authToken();const x=await fetch("/api/focus-source",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+token},body:JSON.stringify({url:d.source_url})});const p=await x.json();if(x.ok){setS(`[공식자료] ${p.title}\n제목: ${p.title}\n공식URL: ${p.url}\n\n${p.text}`);setSaved("공식 URL에서 원문을 다시 확인해 불러왔습니다.");}else setSaved("공식 URL 재수집은 실패했지만 저장된 원문을 불러왔습니다. "+(p.error||""));}catch(e){setSaved("공식 URL 재수집은 실패했지만 저장된 원문을 불러왔습니다. "+e.message)}}}catch(e){setSaved(e.message)}finally{setImporting(false)}})()},[]);

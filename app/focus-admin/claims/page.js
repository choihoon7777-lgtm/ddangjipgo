...
const claimLabel={open:"접수",reviewing:"검토중",resolved:"처리완료",rejected:"반려"};
export default function Claims(){
 const[items,setItems]=useState([]),[notes,setNotes]=useState({}),[filter,setFilter]=useState("open"),[busy,setBusy]=useState(""),[msg,setMsg]=useState(""),[confirmTask,setConfirmTask]=useState(null);
 async function load(){
 ...
 {!items.length?<div className="adminEmpty"><b>해당 상태의 정정 요청이 없습니다.</b><span>독자 정정 요청이 접수되면 기사·근거·처리이력을 이곳에서 관리합니다.</span></div>:items.map(x=><article className="dfClaimCard" key={x.id}>
   <div className="dfClaimHead"><div><small>{new Date(x.created_at).toLocaleString("ko-KR")} · {claimLabel[x.status]||x.status}</small><b>{x.df_articles?.title||"기사 연결 없음"}</b></div>{x.article_id&&<a href={"/focus/article?id="+x.article_id} target="_blank" rel="noreferrer">기사보기 ↗</a>}</div>
   <p>{x.issue_text}</p>
   ...
   <div className="dfReviewActions">
    ...
    <button className="adminPrimary" disabled={!!busy} onClick={()=>setConfirmTask({x,status:"resolved",title:"정정 요청 처리완료",body:"처리 메모를 기준으로 이 요청을 완료 상태로 변경합니다.",label:"처리완료"})}>처리완료</button>
    <button className="adminSecondary" disabled={!!busy} onClick={()=>setConfirmTask({x,status:"rejected",title:"정정 요청 반려",body:"처리 메모를 기준으로 이 요청을 반려합니다.",label:"반려",danger:true})}>반려</button>
   </div>
 </article>)}
 </section><DFToast message={msg} onClose={()=>setMsg("")}/><DFConfirmModal open={!!confirmTask} title={confirmTask?.title} body={confirmTask?.body} confirmLabel={confirmTask?.label} danger={confirmTask?.danger} onCancel={()=>setConfirmTask(null)} onConfirm={()=>{const t=confirmTask;setConfirmTask(null);if(t)setStatus(t.x,t.status)}}/><DFAdminBottomNav active="home"/></main>
}
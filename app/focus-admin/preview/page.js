...
export default function Preview(){
 ...
 async function save(){setBusy(true);setMsg("");const{data,error}=await dfSupabase.rpc("df_save_article_revision",{p_article_id:a.id,p_title:a.title,p_body:body,p_change_note:note.trim()||null});if(error)setMsg(error.message);else{const wasPublic=["published","corrected"].includes(a.status);setA(data);setMsg(wasPublic?"수정사항을 반영하고 이전 버전을 보존했습니다.":"수정사항을 저장했습니다. 내용이 바뀌어 사실·숫자·날짜·법적검토를 다시 확인해야 합니다.");setNote("");await loadVersions(a.id)}setBusy(false)}

...
export default function Login(){
 ...
 return <main className="adminShell dfAdminUnified"><DFAdminHeader title="편집국 로그인" kicker="NEWSROOM ACCESS" back="/my"/>
 <section className="dfAdminLoginHero"><small>DEVELOPMENT FOCUS</small><h1>편집국 운영센터</h1><p>승인된 편집국 운영자만 기사·광고·정정·통계 기능에 접근합니다.</p></section>
 <section className="adminPanel formPanel"><label>이메일<input value={email} onChange={e=>setEmail(e.target.value)} type="email" autoComplete="email"/></label><label>비밀번호<input value={password} onChange={e=>setPassword(e.target.value)} type="password" autoComplete={mode==="login"?"current-password":"new-password"}/></label><button className="adminPrimary" disabled={busy||!email||password.length<6} onClick={go}>{busy?"확인 중…":mode==="login"?"로그인":"편집국 계정 만들기"}</button>{msg&&<div className="editorGate"><span>{msg}</span></div>}<button className="adminSecondary" onClick={()=>{setMode(mode==="login"?"signup":"login");setMsg("")}}>{mode==="login"?"처음이면 편집국 계정 만들기":"이미 계정이 있으면 로그인"}</button></section></main>
}
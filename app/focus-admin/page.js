...
export default function Admin(){
 const[stats,setStats]=useState({queue:null,published:null,claims:null});
 useEffect(()=>{let live=true;(async()=>{const{data:{user}}=await dfSupabase.auth.getUser();if(!user){location.href="/focus-admin/login";return}const [q,p,c]=await Promise.all([dfSupabase.from("df_editorial_queue").select("id",{count:"exact",head:true}),dfSupabase.from("df_articles").select("id",{count:"exact",head:true}).eq("status","published"),dfSupabase.from("df_claims").select("id",{count:"exact",head:true})]);if(live)setStats({queue:q.count??null,published:p.count??null,claims:c.count??null})})();return()=>{live=false}},[]);
 ...
 <section className="adminToday"><div className="adminSectionTitle"><div><small>TODAY</small><h2>오늘의 운영상황</h2></div><span>실데이터</span></div><div className="adminStats"><div><b>{stats.queue??"—"}</b><span>검토대기</span></div><div><b>{stats.published??"—"}</b><span>발행기사</span></div><div><b>—</b><span>광고진행</span></div><div><b>{stats.claims??"—"}</b><span>정정요청</span></div></div></section>

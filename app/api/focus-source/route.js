...
const allowedExact=new Set(["korea.kr","law.go.kr","www.law.go.kr","dart.fss.or.kr","kind.krx.co.kr","www.krx.co.kr","data.go.kr","www.data.go.kr"]);
function allowedHost(host,registered=new Set()){
 const h=host.toLowerCase().replace(/^www\./,"");
 return allowedExact.has(host.toLowerCase())||h.endsWith(".go.kr")||h.endsWith(".re.kr")||registered.has(h);
}
...
export async function POST(req){
 try{
  ...
  const scoped=createClient(SUPABASE_URL,SUPABASE_KEY,{global:{headers:{Authorization:"Bearer "+token}},auth:{persistSession:false,autoRefreshToken:false}});
  const{data:registeredSources}=await scoped.from("df_sources").select("base_url").eq("is_active",true);
  const registered=new Set((registeredSources||[]).map(x=>{try{return new URL(x.base_url).hostname.toLowerCase().replace(/^www\./,"")}catch{return""}}).filter(Boolean));
  if(u.protocol!=="https:"||!allowedHost(u.hostname,registered))return NextResponse.json({error:"등록된 정부·공공기관 공식 도메인만 불러올 수 있습니다."},{status:400});
  ...
  let finalUrl;try{finalUrl=new URL(res.url||u.toString())}catch{return NextResponse.json({error:"최종 공식 URL 확인에 실패했습니다."},{status:502})}
  if(finalUrl.protocol!=="https:"||!allowedHost(finalUrl.hostname,registered))return NextResponse.json({error:"공식 도메인 밖으로 이동된 URL은 차단했습니다."},{status:400});

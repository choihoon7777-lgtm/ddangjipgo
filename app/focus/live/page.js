...
export default function Live(){
 ...
 useEffect(()=>{(async()=>{setLoading(true);let q=dfSupabase.from("df_articles").select("id,title,subtitle,category,region_code,published_at,updated_at").in("status",["published","corrected"]).order("published_at",{ascending:false}).limit(30);if(category!=="전체")q=q.eq("category",category);const{data}=await q;setItems(data||[]);setLoading(false)})()},[category]);

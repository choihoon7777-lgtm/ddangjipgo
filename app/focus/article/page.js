...
function ArticleInner(){
 const[a,setA]=useState(null),[sources,setSources]=useState([]),[history,setHistory]=useState([]),[loading,setLoading]=useState(true),[saved,setSaved]=useState(false),[user,setUser]=useState(null),[saveMsg,setSaveMsg]=useState(""),[saveBusy,setSaveBusy]=useState(false);
 ...
 return <main className="focusShell dfHigh">
  ...
  <article className="dfNewsArticle">
   ...
   <div className="dfArticleActions"><button disabled={saveBusy} aria-pressed={saved} onClick={toggleSave}>{saveBusy?"처리 중…":saved?"★ 저장됨":"☆ 기사 저장"}</button><a href={"/focus/correction?id="+a.id}>정정 요청</a></div>
   {saveMsg&&<p className="dfArticleActionMsg">{saveMsg}</p>}

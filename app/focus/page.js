...
export default function FocusHome(){
 ...
 return <main className="focusShell dfHigh"><DFBrandHeader/>
  ...
  <section className="focusBlock dfHomeLead">
   ...
   </>:<div className="focusEmpty"><b>발행 준비 중입니다</b><span>검증·승인된 기사만 이곳에 노출됩니다.</span></div>}
  </section>
  ...
  <section className="focusBlock dfLatestBlock"><DFSectionTitle eyebrow="LATEST" title="최신 개발뉴스" href="/focus/live"/>
   {latest.length?latest.map(a=><a className="dfLatestRow" href={"/focus/article?id="+a.id} key={a.id}><div><small>{a.category||"최신뉴스"}{a.region_code?" · "+a.region_code:""}</small><h3>{a.title}</h3><span>{dateLabel(a.published_at)} · {a.updated_at&&a.updated_at!==a.published_at?"수정기사 · ":""}공식자료 기반</span></div><b>→</b></a>):news.length>0&&<div className="focusEmpty"><span>추가 기사를 준비 중입니다.</span></div>}
  </section>
  <section className="focusBlock dfRegionHome"><DFSectionTitle eyebrow="REGIONAL" title="지역별 FOCUS" href="/focus/region" label="전국보기 →"/><p className="dfRegionHomeDesc">관심 지역을 선택하면 개발사업·고시·분양·금융 뉴스를 한곳에서 볼 수 있습니다.</p><div className="focusRegions">{regions.map(x=><a href={"/focus/region?name="+encodeURIComponent(x)} className="focusRegionCard" key={x}><b>{x}</b><span>FOCUS</span><small>개발정보 →</small></a>)}</div></section>

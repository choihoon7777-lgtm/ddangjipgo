...
export default function Recent(){
 ...
 useEffect(()=>{load()},[]);
 async function clearAll(){if(!confirm("최근 본 기사 기록을 모두 삭제할까요?"))return;const{data:{user}}=await dfSupabase.auth.getUser();if(!user)return;const{error}=await dfSupabase.from("df_recent_articles").delete().eq("profile_id",user.id);if(error)setMsg(error.message);else load()}
 return <main className="focusShell dfHigh"><DFSubHeader title="최근 본 기사" kicker="HISTORY" back="/my" right={null}/><section className="focusBlock"><div className="focusTitle"><div><small className="focusEyebrow">HISTORY</small><h2>열람 기록</h2></div>{items.length>0&&<button className="miniBtn" onClick={clearAll}>전체삭제</button>}</div>{msg&&<p className="adminDataNote">{msg}</p>}{items.length?items.map(x=>{const a=x.df_articles;return <a className="focusNews" href={"/focus/article?id="+x.article_id} key={x.article_id}><div><small>{a.category}{a.region_code?" · "+a.region_code:""}</small><h3>{a.title}</h3><p>{new Date(x.last_viewed_at).toLocaleString("ko-KR")}</p></div><span className="newsArrow">→</span></a>}):<div className="focusEmpty"><b>최근 본 기사가 없습니다.</b><span>로그인 상태에서 읽은 기사가 자동으로 기록됩니다.</span></div>}</section><DFBottomNav active="my"/></main>
}
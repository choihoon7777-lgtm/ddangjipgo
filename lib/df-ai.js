...
async function callDirectOpenAI(instructions,input,model,key){
 const r=await fetch(OPENAI_ENDPOINT,{method:"POST",headers:{"Authorization":"Bearer "+key,"Content-Type":"application/json"},body:JSON.stringify({model,instructions,input})});
 if(!r.ok)throw new Error("AI_REQUEST_FAILED_"+r.status);
 const j=await r.json();
 return j.output_text||j.output?.flatMap(x=>x.content||[]).filter(x=>x.type==="output_text").map(x=>x.text).join("\n")||"";
}

async function callGateway(instructions,input,model,token){
 const gatewayModel=model.startsWith("openai/")?model:"openai/"+model;
 const r=await fetch(GATEWAY_ENDPOINT,{method:"POST",headers:{"Authorization":"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({model:gatewayModel,messages:[{role:"system",content:instructions},{role:"user",content:input}],stream:false})});
 if(!r.ok){
  let detail="";try{detail=(await r.text()).slice(0,300)}catch{}
  throw new Error("AI_GATEWAY_FAILED_"+r.status+(detail?"_"+detail:""));
 }
 const j=await r.json();
...
async function resolveGatewayToken(){
 ...
 try{
  ...
  return await getVercelOidcToken({project:"prj_mbS3kBWI0cYB9QTDmoFscdQB6ElL",team:"team_wAsV551VHlOwBpqCfY6hPXMg"});
 }catch{return null}
}

async function callAI(instructions,input,model){
 const directKey=process.env.OPENAI_API_KEY;
 const chosen=model||process.env.DF_AI_MODEL||"gpt-5.4-mini";
 if(directKey)return callDirectOpenAI(instructions,input,chosen,directKey);
 const gatewayToken=await resolveGatewayToken();
 if(gatewayToken)return callGateway(instructions,input,chosen,gatewayToken);
 throw new Error("AI_AUTH_NOT_CONFIGURED");
}
...
export async function buildArticle(source){
 ...
 return{fact,draft,article:desk,verification:verify,models:{fact:"gpt-5.4-nano",reporter:"gpt-5.4-mini",desk:"gpt-5.4-mini",verify:"gpt-5.4-nano"},transport:process.env.OPENAI_API_KEY?"openai-direct":"vercel-ai-gateway"};
}

...
const domainOf=req=>`${req.headers['x-forwarded-proto']||'https'}://${req.headers['x-forwarded-host']||req.headers.host||'localhost'}`;const get=(url,accept='application/json,text/plain,*/*',domain='https://localhost')=>new Promise((resolve,reject)=>{const u=new URL(url),r=https.request({protocol:u.protocol,hostname:u.hostname,path:u.pathname+u.search,headers:{'User-Agent':'JITGO/1.0','Accept':accept,'Referer':domain+'/'},timeout:8000},x=>{let t='';x.setEncoding('utf8');x.on('data',d=>t+=d);x.on('end',()=>x.statusCode>=200&&x.statusCode<300?resolve(t):reject(new Error('HTTP_'+x.statusCode)))});r.on('timeout',()=>r.destroy(new Error('TIMEOUT')));r.on('error',reject);r.end()});
const json=async u=>JSON.parse(await get(u,'application/json,text/plain,*/*',domain));
...
module.exports=async function handler(req,res){
 ...
 const key=process.env.VWORLD_API_KEY,address=String(req.query.address||'').trim().replace(/\s+/g,' '),domain=domainOf(req);
 ...
 try{
  const g=new URL('https://api.vworld.kr/req/address');g.search=new URLSearchParams({service:'address',request:'getcoord',version:'2.0',crs:'EPSG:4326',address,refine:'true',simple:'false',format:'json',type:'PARCEL',key,domain});
  ...
  const purl=new URL('https://api.vworld.kr/req/data');purl.search=new URLSearchParams({service:'data',request:'GetFeature',data:'LP_PA_CBND_BUBUN',key,domain,format:'json',size:'10',page:'1',geometry:'true',attribute:'true',crs:'EPSG:4326',geomFilter:`POINT(${lon} ${lat})`});
  ...
  const lu=new URL('https://api.vworld.kr/ned/data/getLandUseAttr');lu.search=new URLSearchParams({pnu,format:'json',key,domain,numOfRows:'1000'});const uses=arr(await json(lu)),all=uniq(uses.map(x=>pick(x,['prposAreaDstrcCodeNm','prposAreaDstrcNm','prposAreaDstrc','uname','unm','name'])));const zones=all.filter(n=>/(주거|상업|공업|녹지|관리|농림|자연환경보전)지역/.test(n));
  let ledger={};try{const u=new URL('https://api.vworld.kr/ned/data/ladfrlList');u.search=new URLSearchParams({key,pnu,format:'xml',numOfRows:'10',pageNo:'1',domain});const x=await get(u,'application/xml,text/xml,*/*',domain);ledger={type:tag(x,'lndcgrCodeNm'),area:Number(tag(x,'lndpclAr'))||null,updated:tag(x,'lastUpdtDt')}}catch(e){}
